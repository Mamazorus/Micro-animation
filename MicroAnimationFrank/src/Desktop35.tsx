...
type Spec = 'code' | 'graphisme' | 'uiux' | 'marketing' | 'finance' | 'autre'

const SPECIALITES: { id: Spec; label: string }[] = [
  { id: 'code',       label: 'Code'      },
...
function SpecBtn({ id, label, active, onClick }: { id: Spec; label: string; active: boolean; onClick: () => void }) {
  return (
    <button
      ...
      onClick={id === 'graphisme' ? undefined : onClick}
      aria-pressed={active}
    >
      {/* Fond card */}
      ...
      <div className="sp-icon-box">
        ...
        {id === 'graphisme' && <GdIcon       active={active} onToggle={onClick} />}
